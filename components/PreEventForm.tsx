...
export const PreEventForm: React.FC<Props> = ({ data, onChange, onRunTest }) => {
  ...
  const inputClass = "w-full p-2 border rounded bg-gray-700 text-white border-gray-600 placeholder-gray-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500";
    ...
    <div className="space-y-8 pb-20">
      ...
      <section className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex gap-2 mb-4 flex-col sm:flex-row">
          <input 
            placeholder="Name" 
            className="border p-2 rounded flex-1 bg-gray-700 text-white border-gray-600 placeholder-gray-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            value={newJudge.name || ''}
      ...
      <section className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex gap-2 mb-4 flex-col md:flex-row items-end">
          <div className="w-full md:w-1/3">
             ...
             <input className="border p-2 rounded w-full bg-gray-700 text-white border-gray-600 placeholder-gray-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" value={newOrg.name || ''} onChange={e => setNewOrg({...newOrg, name: e.target.value})} />
          </div>
          <div className="w-full md:w-1/3">
             <label className="text-xs font-semibold text-gray-500">Phone</label>
             <input className="border p-2 rounded w-full bg-gray-700 text-white border-gray-600 placeholder-gray-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" value={newOrg.phone || ''} onChange={e => setNewOrg({...newOrg, phone: e.target.value})} />
          </div>
          <div className="w-full md:w-1/3">
            <label className="text-xs font-semibold text-gray-500">Role</label>
            <select className="border p-2 rounded w-full bg-gray-700 text-white border-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" value={newOrg.role} onChange={e => setNewOrg({...newOrg, role: e.target.value as OrganizerRoleType})}>
          ...
          {data.criteria.map((crit) => (
            <div key={crit.id} className="border p-3 rounded bg-gray-50">
              <div className="flex justify-between mb-2">
                <input 
                  className="font-bold bg-gray-700 text-white border border-gray-600 rounded px-2 py-1 w-1/2 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" 
                  value={crit.name}
              ...
              <textarea 
                className="w-full text-sm p-2 border rounded mb-2 bg-gray-700 text-white border-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" 
              ...
              <div className="flex items-center text-xs text-gray-500">
                <span>Scale: </span>
                <input 
                   className="ml-2 bg-gray-700 text-white border border-gray-600 rounded px-1 w-16 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                   value={crit.scale}
